...
7. Note the Master Service Endpoint option. This controls how you (or anyone) can access your cluster for admin purposes ie via the command line. Select "Both Public and Private Endpoints". This simplifies the rest of the build but note that you can disable the public endpoint and this then requires you to connect in via VPN (see VPN options in the Virtual Private Cloud docs). This would be desirable in a critical production system. You can enable or disable the public endpoint at any time later.
8. Give your cluster a name in the next field, check the estimated cost on the right hand side and check/adjust your workers to achieve the correct cost then click Create. The cluster creation will take some time.
9. Once your cluster is up and running, if not already installed, install the Block Storage for VPC plugin via the Getting Started page for your cluster.
10. Check/view your cluster by clicking on the Kubernetes Dashboard button from the Overview page of your cluster. Keep this dashboard open for use later. 
11. Connect from your command line/terminal to your cluster as described in the Access page of your cluster (this is now available via the Actions button by clicking the Connect via CLI option). You need to install helm [Link](https://helm.sh/docs/intro/install/) and the IBM Cloud Command line tool (as described in the Access page), if you haven't already done this (one time process). 

The following steps 12 to 18 are taken from, or modified from the Bitnami documentation here https://docs.bitnami.com/tutorials/secure-wordpress-kubernetes-managed-database-ssl-upgrades/ 

12. Install the NGINX ingress controller. This creates a Virtual Public Cloud load balancer that you can see under the Load Balancers section of the VPC Infrastructure menu in IBM Cloud.
```
helm repo add bitnami https://charts.bitnami.com/bitnami
helm install ingress bitnami/nginx-ingress-controller
```
13. Obtain the LoadBalancer address by issuing this command. As it is pointing to a load balancer the address will not be a IP address, it will be a in the form abc123-eu-gb.lb.appdomain.cloud.
```
kubectl get svc ingress-nginx-ingress-controller
```
14. Configure the DNS for your domain name by adding a CNAME record pointing to the address obtained above (eg abc123-eu-gb.lb.appdomain.cloud). This may take a few minutes or a few hours to take effect (up to 24 hours).

15. Add the cert-manager repository, create a namespace and create CRDs. New versions are released al the time so it's worth checking for the latest version. Go to https://github.com/jetstack/cert-manager and look at the "Releases" section on the right hand side of the page. The instruction below assumes v1.3.1 is the latest version. 
```
...
22. The cert-manager will go off to the Let's Encrypt service and generate SSL certificates for you and then manage them for you! If your DNS update hasn't taken effect yet, cert-manager can't go and make that request and you'll see an extra pod running in your Kubernetes dashboard (called something like cm-acme-http-solver-xxxx) until it can go and do this. NOTE: It can take ages for a update to take effect so don't worry, you may have to wait a few hours.
23. When this all completes you should be able to access Wordpress via your domain name. It may take a while for this complete.
24. The W3 Total Cache plugin will be installed and generally set up. Review the settings and note that memcached is set/available in the list of caching servers. 
25. Now what about if you want to be able to connect your Wordpress instance using www.mydomain.com as well as mydomain.com. This is a common requirement. Searching for this on the internet will return a lot of scary looking processes but in this case, basically nginx and cert-manager take care of this, you just need to add a few lines to the ingress definition....
